...
async function postToSlack(articles, today) {
  ...
  const articleLines = top10.map(a =>
    `${heatEmoji(a.heat)} *${a.heat}/10* <${a.url}|${a.title}> — _${a.source}_`
  ).join('\n');

  const text = `*🗞️ AI Legal Tech Daily — ${today}*\n*${articles.length} articles* • *${hotCount} 🔥 hot stories*\n\n${articleLines}\n\n📊 <${dashboardUrl}|View full dashboard> — ${articles.length - 10} more articles inside`;
